...
% C and contradict this table. The Blockbuster column is a single five-fold run.
% =============================================================================
...
\begin{table}[t]
  ...
  \caption{Genre classification through the predicted-emotion bottleneck versus the raw
    ...
    Eerola column is the $10\times5$ repeated GroupKFold result with nested-CV-tuned
    regularisation (Section~\ref{subsec:statistical_power}); the Blockbuster column is a
    single five-fold run, the only measurement available for that corpus.}
  \label{tab:emotion_transfer}
  \begin{tabular}{lrr}
    \hline
    Approach & Eerola (5 genres) & Blockbuster (6 genres) \\
    \hline
    predicted emotion $\rightarrow$ genre & \textbf{0.394} & 0.565 \\
    ground-truth emotion $\rightarrow$ genre & 0.397 & --- \\
    PCA-8 $\rightarrow$ genre (control) & 0.343 & 0.559 \\
    raw VGGish $\rightarrow$ genre & 0.350 & 0.582 \\
    most-frequent baseline & 0.166 & 0.058 \\
    \hline
  \end{tabular}
\end{table}
...
film-level Blockbuster task the emotion bottleneck instead ties the control
($0.565$ vs $0.559$). Second, the predicted emotions are as genre-informative as the human
