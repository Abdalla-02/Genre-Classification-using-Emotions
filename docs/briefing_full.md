# Supervisor briefing — full reference (all rounds)
...
> **For the latest meeting only, read `current_state.md`.** This is the complete reference:
> every round of notes, the definitions, how the evaluation works, and every result. It
> supersedes `current_state.docx` / `.pdf`, which are exports of an older version. The
> LaTeX in `docs/latex/` is for pasting into the thesis, and the full technical log with
> every experiment is `docs/README.md`.
...
| what | where |
|---|---|
| This full briefing | `docs/briefing_full.md` |
| The update for the latest meeting | `docs/current_state.md` |
